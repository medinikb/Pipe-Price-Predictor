...
function getSelectedRawMaterialItem() {
  ...
  return (group.children || []).find(
    (_, index) => String(index) === elements.materialGradeFamily.value
  );
}
...
function populateRawMaterialGradeOptions() {
  const group = getSelectedRawMaterialGroup();
  elements.materialGradeFamily.innerHTML = '<option value="">Select grade family</option>';
  ...
  (group.children || []).forEach((item, index) => {
    ...
    option.textContent = getRawMaterialGradeLabel(item);
    elements.materialGradeFamily.append(option);
  });
  resetRawMaterialLibraryOutputs("Select grade family");
}
...
function refreshRawMaterialLibrarySelection(previousBasis = "", previousGrade = "") {
  ...
  if (previousGrade) {
    elements.materialGradeFamily.value = previousGrade;
  }
...
function applyRawMaterialPriceSelection() {
  ...
  if (!item) {
    resetRawMaterialLibraryOutputs(elements.materialBasis.value ? "Select grade family" : "-");
    clearRawMaterialPriceSelection();
    return;
  }
  ...
  if (Number.isFinite(recommended) && recommended > 0) {
    ...
    elements.spec.value = standardList[0] || "Other spec";
...
function calculate() {
  ...
  if (hasIncompleteNonCarbonSteelSelection()) {
    elements.warning.textContent =
      "Select Ch.Comp / Grade Family before estimating non-carbon-steel pipe.";
    return null;
  }
...
function addCurrentLine() {
  if (hasIncompleteNonCarbonSteelSelection()) {
    const message = "Please select Ch.Comp / Grade Family before adding non-carbon-steel pipe.";
